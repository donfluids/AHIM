...
- **`index.html`** — the entire site (HTML, CSS, and a few lines of JavaScript in one file; no build step, no dependencies).
  ...
  - Service times (Sunday Worship 10:00 AM – 2:00 PM, midweek prayer, special gatherings)
...
- **Featured video** — replace the video ID in `https://www.youtube.com/embed/bqKz5bnweMQ` with any YouTube video ID (or use `https://www.youtube.com/embed/live_stream?channel=UCg378OhEuTkIspByJErUHng` while streaming live).
...
2. Under **Build and deployment**, choose **Deploy from a branch**, select the default branch and the `/ (root)` folder, then save.
3. The site will be live at `https://<username>.github.io/churchrepo/` within a couple of minutes.

A custom domain (e.g. `aronhabritministries.org`) can be pointed at GitHub Pages under the same settings page.
